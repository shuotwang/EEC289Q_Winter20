...
int** generateData(int numArray, int maxArrayLen, int maxValue, int* eachLen){
	...
	// ciramdSetPseudoRandomGeneratorSeed();
...
__global__
void kernelSetRandom(curandState *curandStates, int num, long clock_for_rand){
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx > num)	return;
	curand_init(clock_for_rand, idx, 0, &curandStates[idx]);
}

__global__
void findPos(int** valueArr, int** thisPosArr, int** nextPosArr, int** data, int numArray, int* newLen, int* oriLen, int N, curandState* curand_states){
	...
	*/
	...
	curandState localState = curand_states[a];
...
int main(int argc, const char * argv[]) {
	...
	cudaMalloc((void**) &dev_states, sizeof(curandState) * N);
    ...
    findPos<<<numBlock, blockSize>>>(d_valueArr, 
								     ...
								     d_nextPosArr, 
									 d_data, 
									 numArray, newLenK, eachLenK, N, dev_states);
